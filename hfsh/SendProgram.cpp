...
int SendProgram::httpSend(const OrderTarget &target, const std::string &token, Response& res,std::string &desc)
{
  try
  {
    //专门处理节目的目录
    ...
    std::shared_ptr<cJSON> resJson;
    int http_status_code = OrderRequest::httpSend(target, token, resJson, desc);

    if(http_status_code != 200)
    {
        desc = "http_status_code = " + std::to_string(http_status_code);
        return -1;
    }

    if(resJson == nullptr)
    {
        desc = "recv json body error";
        return -1;
    }

    bool parse_ok = res.parseWebJsonInput(resJson.get());
    if (!parse_ok)
    {
        desc = "recv json body inconsistency";
        std::string json = JsonHandler::JsonToString(resJson);
        TRACE_LOG_A(nullptr, TRACE_LOG_LEVEL_ERROR, "inconsistency json: %s",json.c_str());
        return -1;
    }
